sap.ui.define([
    ...
    return Controller.extend("com.gcc.reports.ns01.ns01report.controller.report", {
        ...
        _onRouteMatched: function () {
            ...
            oModel1.dataLoaded()
                .then(() => {
                    ...
                    this._newStarter = this.byId("empName_ID")
        ...
        forInitiator: async function (useremail) {
            try {
                ...
                let cust_Value, values;
                await $.ajax({
                    url: ajaxUrl + "/odata/v2/cust_ZFLM_MULTI_USERS?$filter=cust_UserName eq '" + initiator + "'&$format=json",
                    type: 'GET',
                    contentType: "application/json",
                    success: function (data) {
                        console.log("success" + data);
                            ...
                            values.forEach(function (oItemData) {
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        oComboBox.setSelectedKey(values[0].text);
                                    },
                                    error: function (e) {
                                        console.log("error: " + e);
                                    }
                            ...
                            values.forEach(function (oItemData) {
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        oComboBox.addItem(new sap.ui.core.Item({
                                            key: data.d.results[0].externalCode,
                                            text: data.d.results[0].externalName
                                        }));
                                    },
                                    error: function (e) {
                                        console.log("error: " + e);
        ...
        onOpenForm: function (oEvent) {
            ...
            sap.ui.require(["sap/ushell/Container"], async (Container) => {
                ...
                var sHref = await (oNavigationService && oNavigationService.getHref({
                    target: { semanticObject: "GCC_SemObj_NewStarterQA", action: "Display" },
            ...
            // var reqUrl = appUrl.includes("GCC_SemObj") ? `https://gccdev.launchpad.cfapps.eu10.hana.ondemand.com/site?siteId=1c5358bf-4495-47e5-96c0-a07e5b706991#GCC_SemObj-Display?sap-ui-app-id-hint=saas_approuter_project1&/${queryParameters}` : `https://gccdev.launchpad.cfapps.eu10.hana.ondemand.com/adf63da7-2c78-4e08-bcea-845aec738eef.project1.project1-0.0.1/index.html#${queryParameters}`;
        ...
        onGo: function (oFilterLog1, oFilterLog2, oFilterApprover) {
            var ifOrg = this._fOrg.getSelectedItem(),
                ...
                iemp_name = this._newStarter.getValue()
            ...
            if (iemp_name) {
                oFilterNS01.push(new Filter("NewEmployee", FilterOperator.EQ, iemp_name));
            }
            ...
            this.getOwnerComponent().getModel("ZSFGTGW_LOG_SRV").read("/zsf_logSet", {
                ...
                success: function (oData) {
                    this.fetchAdditinonalDataForItems(oData.results, oFilterNS01);
                }.bind(this),
            ...
            oEvent.getSource().getFilterItems().forEach(function (oValue) {
                ...
                if (filterValue) {
                    if (filterKey == 0) {
                        ...
                        oFilterLog1.push(new Filter("DateCreated", FilterOperator.EQ, filterValue));
                    } else if (filterKey == 3) {
                        ...
                        oFilterLog1.push(new Filter("NewStarter", FilterOperator.EQ, filterValue));
                    } else if (filterKey == 6) {
                        oFilterLog1.push(new Filter("Position", FilterOperator.EQ, filterValue));
                    } else if (filterKey == 7) {
                        oFilterLog1.push(new Filter("StartDate", FilterOperator.EQ, filterValue));
                    } else if (filterKey == 8) {
                        oFilterLog1.push(new Filter("Grade", FilterOperator.EQ, filterValue));
                    } else if (filterKey == 9) {
                        oFilterLog1.push(new Filter("Scale", FilterOperator.EQ, filterValue));
                    } else if (filterKey == 10) {
                        oFilterLog1.push(new Filter("Hours", FilterOperator.EQ, filterValue));
                    }
                }
            })
        ...
        fetchAdditinonalDataForItems: function (aItems, filter) {
            ...
            if (aItems.length != 0) {
                aItems.forEach(function (oItem) {
                    ...
                    this.getOwnerComponent().getModel("ZSFGTGW_NS01_SRV").read("/ZSFGT_NS01Set(Formid='" + FormId + "')", {
                        filters: [filter],
                        success: function (oData) {
                            var temp = {
                                "Form_ID": FormId,
                                "Status": Status,
                                "DateCreated": StartedOn,
                                "Initiator": Initiator,
                                "Approver": "",
                                "NewStarter": oData.Forename + " " + oData.Surname,
                                "Position": oData.PositionTitle,
                                "StartDate": oData.StartDate,
                                "Grade": oData.Grade,
                                "Scale": oData.ScalePoint,
                                "Hours": oData.NoHours,
                                "Owner": oItem.FormOwner,
                                "StartedBy": oItem.Initiator
                            }
                            tableData.push(temp);
        ...
        _registerForP13n: function () {
            ...
            this.oMetadataHelper = new MetadataHelper([
                ...
                { key: "NewStarter", label: "New Starter Name", path: "AllData>NewStarter" },
                { key: "Position", label: "Position", path: "AllData>Position" },
                { key: "StartDate", label: "Start Date", path: "AllData>StartDate" },
                { key: "Grade", label: "Grade", path: "AllData>Grade" },
                { key: "Scale", label: "Scale", path: "AllData>Scale" },
                { key: "Hours", label: "Hours", path: "AllData>Hours" },
                { key: "Owner", label: "Owner", path: "AllData>Owner" },
                { key: "StartedBy", label: "Started By", path: "AllData>StartedBy" }
            ]);
        ...
        _openDialog: function (sName, sPage, fInit) {
            ...
            if (!this._mDialogs[sName]) {
                this._mDialogs[sName] = Fragment.load({
                    id: oView.getId(),
                    name: "com.gcc.reports.ns01.ns01report.fragment." + sName,
                    controller: this
        ...
        onSort: function (oEvt) {
            ...
            if (sSortOrder == "Ascending") {
                liveTableData.sort((a, b) => {
                    if (sAffectedProperty == "DateCreated" || sAffectedProperty == "Available_From") {
